...
const labels = { buy: 'خرید', sell: 'فروش', receive: 'دریافت', payment: 'پرداخت', salary: 'حقوق', expense: 'هزینه', debt: 'بدهی', credit: 'طلب' };
...
async function generateReport({ resetDates = false } = {}) {
    try {
        ...
        const allTransactions = await window.boutiaDB.getAll('transactions');
        populateDateOptions(allTransactions, resetDates);
        const from = $('fromDate').value.trim() ? parseShamsi($('fromDate').value).canonical : '';
        const to = $('toDate').value.trim() ? parseShamsi($('toDate').value).canonical : '';
        if (from && to && from > to) throw new Error('تاریخ شروع نباید بعد از تاریخ پایان باشد.');
        const type = $('typeFilter').value;
        const transactions = allTransactions.filter(t => {
            const date = transactionDate(t);
            return (!from || date >= from) && (!to || (date && date <= to)) && (type === 'all' || t.type === type);
        }).sort((a, b) => transactionDate(b).localeCompare(transactionDate(a)) || Number(b.timestamp || 0) - Number(a.timestamp || 0));
        let income = 0, expenses = 0, sold = 0;
        for (const t of transactions) {
            const amount = Number(t.total_amount) || 0;
            const paid = Number(t.paid_amount) || 0;
            if (t.type === 'sell') { income += paid; sold += (t.items || []).reduce((sum, item) => sum + (Number(item.qty) || 0), 0); }
        ...
        $('reportTableBody').innerHTML = transactions.length ? transactions.map((t, i) => {
            const items = (t.items || []).map(item => `${item.category || ''}${item.qty ? ` (${number(item.qty)} عدد)` : ''}`).join('، ');
            const remaining = ['buy', 'sell'].includes(t.type) ? (Number(t.total_amount) || 0) - (Number(t.paid_amount) || 0) : 0;
            return `<tr><td>${number(i + 1)}</td><td>${escapeHTML(transactionDate(t) || '-')}</td><td>${t.timestamp ? new Date(t.timestamp).toLocaleTimeString('fa-IR', { hour: '2-digit', minute: '2-digit' }) : '—'}</td><td><span class="badge badge-${Object.hasOwn(labels, t.type) ? t.type : 'expense'}">${escapeHTML(labels[t.type] || t.type)}</span></td><td>${escapeHTML(t.party_name || '-')}</td><td title="${escapeHTML(items)}">${escapeHTML(items.length > 30 ? items.slice(0, 30) + '…' : items)}</td><td>${money(t.total_amount)}</td><td>${money(t.paid_amount)}</td><td>${money(remaining)}</td><td class="description-cell">${escapeHTML(t.description || '-')}</td></tr>`;
        }).join('') : '<tr><td colspan="10" class="empty-state">هیچ تراکنشی در بازه انتخاب‌شده یافت نشد.</td></tr>';
    } catch (error) {
        $('reportTableBody').innerHTML = `<tr><td colspan="10" class="empty-state">${escapeHTML(error.message)}</td></tr>`;
        for (const id of ['totalIncome', 'totalExpenses', 'netProfit', 'totalTransactions', 'totalItemsSold']) $(id).textContent = '—';
